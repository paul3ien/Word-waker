...
- [ ] `[SETUP]` Créer le crate `ui` comme binary crate dans le workspace (`cargo new --bin ui`)
- [ ] `[SETUP]` Ajouter `ui` dans le `[workspace]` du `Cargo.toml` racine
- [ ] `[SETUP]` Créer l'arborescence : `src/`, `src/config.rs`, `src/error.rs`, `src/app.rs`, `src/status_item.rs`, `src/socket_client.rs`, `src/delegate.rs`
- [ ] `[SETUP]` Créer le dossier `tests/` pour les tests d'intégration
- [ ] `[TEST-U]` **Test de smoke :** `cargo check -p ui` — passe sans erreur
...
- [ ] `[SETUP]` Ajouter `name = "word-waker-ui"` dans `[[bin]]` pour que le binaire ait un nom distinct du daemon
- [ ] `[TEST-U]` **Test :** `cargo build -p ui` — compile sans erreur

### P0.3 — Configuration runtime

- [ ] `[SETUP]` Créer `src/config.rs` avec la struct `UiConfig` :
...
- [ ] `[IMPL]` Implémenter `UiConfig::from_env()` — lit `WAKEWORD_SOCKET_PATH`, `WAKEWORD_UI_RECONNECT_MS` depuis les variables d'environnement avec valeurs par défaut
- [ ] `[IMPL]` Implémenter `UiConfig::default()`
- [ ] `[TEST-U]` **Test :** `UiConfig::from_env()` avec variables non définies → valeurs par défaut valides
- [ ] `[TEST-U]` **Test :** `UiConfig::from_env()` avec `WAKEWORD_SOCKET_PATH=/tmp/custom.sock` → valeur personnalisée respectée
...
- [ ] `[IMPL]` Créer `src/socket_client.rs` avec l'enum `UiEvent` :
  ```rust
  pub enum UiEvent {
      Connected,
      Disconnected,
  ...
  ```
- [ ] `[TEST-U]` **Test :** `UiEvent` dérive `Debug`, `Send`, `Sync`
- [ ] `[TEST-U]` **Test :** Un `UiEvent::WakeWordDetected` créé avec `Instant::now()` a un timestamp dans les 100 dernières ms

### P1.2 — Connexion socket client

- [ ] `[IMPL]` Implémenter `IpcClient::connect(socket_path: &str) -> io::Result<UnixStream>` — tente de se connecter au socket, retourne `Err` si daemon absent
- [ ] `[IMPL]` Implémenter `IpcClient::read_loop(stream: UnixStream, tx: &Sender<UiEvent>)` :
...
- [ ] `[TEST-I]` **Test :** Mock server envoie `WAKEWORD_DETECTED\n` → `UiEvent::WakeWordDetected` reçu
- [ ] `[TEST-I]` **Test :** Mock server envoie `WAKEWORD_DETECTED` sans `\n` (flush manuel) → toujours reçu après flush
- [ ] `[TEST-I]` **Test :** Mock server ferme la connexion → `UiEvent::Disconnected` reçu
- [ ] `[TEST-I]` **Test :** Socket inexistant → `UiEvent::Disconnected` ou `Error` reçu

### P1.3 — Boucle de reconnexion automatique

- [ ] `[IMPL]` Implémenter `IpcClient::run(tx: Sender<UiEvent>, config: UiConfig)` — boucle infinie :
  ```
  loop {
  ...
  ```
- [ ] `[TEST-I]` **Test :** Daemon mock démarre après 3 s d'attente → le client finit par se connecter et recevoir des événements
- [ ] `[TEST-I]` **Test :** Daemon mock est killé → `Disconnected` puis `Connected` après redémarrage du mock
- [ ] `[TEST-I]` **Test :** Le client ne spamme pas les `Disconnected` — exactement 1 événement par transition d'état

### P1.4 — Résistance aux malformations

- [ ] `[TEST-I]` **Test :** Le daemon envoie des lignes vides → ignorées, pas de crash
- [ ] `[TEST-I]` **Test :** Le daemon envoie une ligne de 10 Ko → ignorée (pas `WAKEWORD_DETECTED`), pas de crash, pas de OOM
- [ ] `[TEST-I]` **Test :** Le daemon envoie des données binaires → pas de crash, pas de panic
- [ ] `[TEST-I]` **Test :** Le daemon ferme la connexion brutalement (RST) → `Disconnected` émis, pas de panic
...
- [ ] `[IMPL]` Créer `src/app.rs` avec `UiApp` :
  - `new(config: UiConfig) -> Result<Self, UiError>`
  - `run(&self)` — lance `NSApplication::run()` (boucle événementielle AppKit)
- [ ] `[IMPL]` Implémenter `NSApplication::sharedApplication()` avec `MainThreadMarker`
- [ ] `[IMPL]` Configurer `LSUIElement = true` via `NSApplication::setActivationPolicy(NSApplicationActivationPolicy::Accessory)` — pas d'icône dans le Dock
- [ ] `[VALID]` Lancer le binaire → aucune fenêtre, rien dans le Dock, processus visible dans Activity Monitor

### P2.2 — NSStatusBar avec emoji

- [ ] `[IMPL]` Créer `src/status_item.rs` avec `MenuBarIcon` :
...
- [ ] `[IMPL]` Créer le `NSStatusItem` via `NSStatusBar::systemStatusBar().statusItemWithLength(NSVariableStatusItemLength)`
- [ ] `[IMPL]` Définir le bouton du status item avec `setTitle_` et `setToolTip_`
- [ ] `[VALID]` Lancer le binaire → emoji `🎤` visible dans la barre de menu (à droite, près de l'horloge)
- [ ] `[VALID]` Survoler l'emoji → tooltip "Word Waker" visible

### P2.3 — Menu contextuel

- [ ] `[IMPL]` Ajouter `MenuBarIcon::build_menu() -> NSMenu` — crée le menu déroulant :
...
- [ ] `[IMPL]` Associer le menu au `NSStatusItem` via `setMenu_`
- [ ] `[VALID]` Clic gauche sur l'emoji → menu apparaît avec "Quitter"
- [ ] `[VALID]` Clic sur "Quitter" → le processus se termine proprement (exit 0)

### P2.4 — AppDelegate (cycle de vie)

- [ ] `[IMPL]` Créer `src/delegate.rs` avec un `AppDelegate` qui implémente :
  - `applicationDidFinishLaunching` — log "UI démarrée"
  - `applicationWillTerminate` — log "UI arrêtée", cleanup (joindre le thread socket)
- [ ] `[IMPL]` Enregistrer le delegate via `NSApplication::setDelegate_`
- [ ] `[VALID]` Quitter via le menu → logs `[INFO] UI arrêtée` visibles dans la console
- [ ] `[VALID]` `leaks -- word-waker-ui` après arrêt → zéro leak NSObject/NSMenu
...
- [ ] `[IMPL]` Le callback du timer `try_recv()` les événements et les applique à l'UI
- [ ] `[TEST-I]` **Test (avec mock, headless) :** Envoyer `WAKEWORD_DETECTED\n` via mock server → le `StatusItem` change d'emoji brièvement (via inspection de l'état interne)
- [ ] `[TEST-I]` **Test :** Mock server s'arrête → `Disconnected` → l'état interne passe à `Disconnected`
- [ ] `[TEST-I]` **Test :** Mock server redémarre → `Connected` → l'état repasse à `Connected`

### P3.2 — États visuels du StatusItem

- [ ] `[IMPL]` Implémenter `MenuBarIcon::update_state(state: &DaemonState)` :
...
- [ ] `[IMPL]` Implémenter le flash via `NSTimer` (ou `dispatch_after` via GCD) — pas de `sleep()` bloquant le thread UI
- [ ] `[VALID]` Lancer daemon + UI → `🎤` visible, tooltip OK
- [ ] `[VALID]` Tuer le daemon (kill) → l'UI passe à `⏸` dans les 5 secondes
- [ ] `[VALID]` Redémarrer le daemon → l'UI repasse à `🎤` automatiquement
- [ ] `[VALID]` Prononcer le mot-clé → flash `🎤✅` visible, puis retour à `🎤`

### P3.3 — Compteur de détections dans le menu

- [ ] `[IMPL]` Ajouter un item de menu `"Détections : 0"` mis à jour à chaque `WakeWordDetected`
- [ ] `[IMPL]` Le compteur persiste jusqu'à fermeture de l'UI (pas de reset)
- [ ] `[VALID]` 3 détections successives → menu affiche `"Détections : 3"`
- [ ] `[TEST-U]` **Test unitaire :** Simuler 5 `WakeWordDetected` → compteur = 5, pas de panic sur overflow

### P3.4 — Gestion des erreurs socket

- [ ] `[IMPL]` En cas d'erreur fatale du thread socket (3 reconnexions échouées), logger l'erreur et continuer (ne pas crasher l'UI)
- [ ] `[TEST-I]` **Test :** Socket path pointe vers un fichier non-socket → `UiEvent::Error` émis, UI pas crashée
- [ ] `[TEST-I]` **Test :** Permissions insuffisantes sur le socket → `Error`, UI continue
...
- [ ] `[IMPL]` Créer `src/panel.rs` avec `DetectionsPanel` :
...
- [ ] `[IMPL]` Créer une `NSPanel` avec `NSWindowStyleMask::Titled | NSWindowStyleMask::Closable | NSWindowStyleMask::UtilityWindow`
- [ ] `[IMPL]` Configurer `setFloatingPanel(true)` et `setHidesOnDeactivate(false)`
- [ ] `[IMPL]` Ajouter un `NSTextView` (ou `NSTableView`) comme vue de contenu avec scroll
- [ ] `[VALID]` Ouvrir la fenêtre via le menu → fenêtre flottante visible, scrollable si >20 détections
- [ ] `[VALID]` Fermer la fenêtre (bouton rouge) → fenêtre masquée, pas quittée

### P4.2 — Intégration avec le StatusItem menu

- [ ] `[IMPL]` Ajouter un item `"Historique..."` dans le menu entre "Détections : N" et "Quitter"
- [ ] `[IMPL]` Le callback de l'item appelle `panel.show()`
- [ ] `[VALID]` Clic "Historique..." → fenêtre apparaît, focus au champ de texte
